...
function keyboardSetup() {
	$("body").keydown(function( event ) {
		switch(event.which) {	
			// up
			case 38: sendCommand("VERTICAL", 1);
				break; 
			// down
			case 40: sendCommand("VERTICAL", -1);
				break;
			// left
			case 37: sendCommand("HORIZONTAL", -1);
				break;
			// right
			case 39: sendCommand("HORIZONTAL", 1);
				break;
		}
	});
	$("body").keyup(function( event ) {
		switch(event.which) {	
			// up
			case 38: sendCommand("VERTICAL", 0);
				break; 
			// down
			case 40: sendCommand("VERTICAL", 0);
				break;
			// left
			case 37: sendCommand("HORIZONTAL", 0);
				break;
			// right
			case 39: sendCommand("HORIZONTAL", 0);
				break;
		}
	});
}
...
function connectWebSocket() {
	...
	var source = "ws://" + streamerUrl + "/websocket?viewerHash=" + viewerHash +
		"&t=" + new Date().getTime();
...
function logOut() {
	if (pollTimeout) clearTimeout(pollTimeout);
	$.ajax({
	    url: "http://" + streamerUrl + "/control?viewerHash=" + viewerHash,
	    dataType: "json",
	    data: {
		"LOGOUT": 1
	    },
...
$(document).ready(function(){
	...
	$("#message").html("<a href=\"#\" onclick=\"logIn(); return false;\">Join the Stream</a>");
});
